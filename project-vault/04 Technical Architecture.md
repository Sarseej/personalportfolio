...
# Blue Meadow rendering update — September 2026

BlueMeadow renders deterministic instanced grass with shared geometry and vertex-shader wind over a gently varying height surface. Standard mode uses 42,000 blades; economy and mobile use 12,000. Broad wind, local variation and slow gusts share spatial phases. Distance shading supplies atmospheric depth without postprocessing.

MeadowFireflies consumes the unchanged signal-field dataset in Home and the standalone field. Twenty shader-lit nodes retain their stable identities and 22 evidence-bearing relationships. World-space positions project into matching 44px semantic buttons each frame without React state updates. Focus holds the corresponding local drift steady. Warm trails reuse preallocated buffers. The field remains in the live Three.js scene after the camera passes the glass; the former flat graph handoff is inactive.

Reduced motion freezes wind, node drift and traveling pulses. Hidden tabs stop continuous rendering and suspend the existing opt-in audio director. Geometry and materials are disposed on unmount. The field and room retain static posters, semantic controls and repository-linked details when WebGL is unavailable. A downloadable PDF is produced from the canonical CV view.
